...
@description('The location for the SQL admin managed identity. Must match the existing identity location if already deployed.')
param managedIdentityLocation string = resourceGroup().location
...
resource puppydb 'Microsoft.Sql/servers/databases@2023-08-01' = {
  name: 'puppydb'
  location: location
  properties: {
...
resource identitydb 'Microsoft.Sql/servers/databases@2023-08-01' = {
  name: 'identitydb'
  location: location
  properties: {
    freeLimitExhaustionBehavior: 'AutoPause'
