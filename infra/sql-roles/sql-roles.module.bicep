...
resource script_sql_puppydb 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('sql', principalName, 'puppydb', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
...
resource script_sql_identitydb 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('sql', principalName, 'identitydb', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
    userAssignedIdentities: {
